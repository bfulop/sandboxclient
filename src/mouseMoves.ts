...
function moveMouse(mouse: HTMLElement) {
...
function safelyGetElement(id: string): IOE.IOEither<{ message: string; }, HTMLElement> {
  return () => {
...
function renderMouse(localenv: MouseStreams): IO.IO<E.Either<{ message: string; }, MouseStreams>> {
  return F.pipe(
...
function pushToServer(throttledMouseEvents: IOE.IOEither<{ message: string; }, Observable<MouseAction>>) {
  return F.pipe(
